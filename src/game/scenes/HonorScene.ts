...
export default class HonorScene extends Phaser.Scene {
  ...
  create() {
    ...
    users.forEach(({ id, data }) => {
      const line = this.add.text(80, y, `${id}  Lv.${data.bestLevel}  徽章:${data.badges.length}  EXP:${data.exp}`, { fontFamily: 'monospace', fontSize: '18px', color: id === cur ? '#2de1c2' : '#a9ffea' })
    ...
    const create = this.add.text(width - 120, 20, '新建用户', { fontFamily: 'sans-serif', fontSize: '16px', color: '#0b1021', backgroundColor: '#ffd166', padding: { x: 8, y: 6 } }).setInteractive({ useHandCursor: true })
    create.on('pointerup', () => {
      const id = prompt('输入新用户ID (字母数字下划线)')?.trim()
      if (id) {
