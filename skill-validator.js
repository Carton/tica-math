...
function validateSkill(question) {
  ...
  switch (skill) {
    ...
    case 'estimate':
      ...
      return {
        valid: correctDigits !== wrongDigits || Math.abs(correctHighest - wrongHighest) >= 5,
...
function generateSkillValidation(digitDifficulty) {
  ...
  skills.forEach(skill => {
    ...
    questions.forEach((q, idx) => {
      const status = q.isValid ? '✓' : '✗'
      console.log(`    ${idx + 1}. ${status} ${q.expression} = ${q.wrong} (正确: ${q.correct})`)
      if (!q.isValid) {
        console.log(`       问题: ${q.validation.detail}`)
      } else {
